...
planets.each do |planet_data|
  begin
    ...
    # Convert diameter and rotation_period to integers
    diameter = diameter.to_i
    rotation_period = rotation_period.to_i

    # Check for valid diameter and rotation_period
    if diameter > 0 && rotation_period > 0
      Planet.create!(
        name: name,
        diameter: diameter,
        rotation_period: rotation_period
      )
      puts "Seeded planet: #{name} with Diameter: #{diameter}, Rotation Period: #{rotation_period}"
    else
      puts "Skipping invalid planet record: Invalid diameter or rotation period for #{name}"
    end
