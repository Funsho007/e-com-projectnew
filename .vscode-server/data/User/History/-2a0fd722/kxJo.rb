...
planets.each do |planet_data|
  begin
    ...
    average_lifespan = planet_data['rotation_period']

    # Convert diameter and average_lifespan to integers
    diameter = diameter.to_i
    average_lifespan = average_lifespan.to_i
    ...
    if diameter > 0 && average_lifespan > 0
      ...
      puts "Seeded planet: #{name} with Diameter: #{diameter}, Average Lifespan: #{average_lifespan}"
    else
      puts "Skipping invalid planet record: Invalid diameter or average lifespan for #{name}"
    end
