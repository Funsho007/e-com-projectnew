...
products_data.first(200).each do |product_data|
  ...
  # Attach image if available with retry mechanism
  begin
    unsplash_image_name = URI.encode_www_form_component(product_data['name'])
    image_url = "https://source.unsplash.com/800x600/?#{unsplash_image_name}"
    image_io = URI.open(image_url)
    product.image.attach(io: image_io, filename: "product_#{product.id}.jpg", content_type: 'image/jpeg')
  rescue Errno::ECONNRESET => e
    puts "Error retrieving image: #{e.message}"
    sleep 5
    retry
